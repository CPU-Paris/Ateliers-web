  ...
  <div>
    ...
    <main>
      ...
      <section v-if="nbSecondes >= 0">
        ...
        <p>
          Vous allez dormir {{ nbJours }} jours !
          <button @click="nbJours--">- jours</button>
          <button @click="nbJours++">+ jours</button>
        </p>
        <achats :items="this.items" class="ok"></achats>
      </section>
      <p v-else class="nope">Fini tu peux plus choisir.</p>

      <p>Prix total : {{ prixPanier }}€</p>
      <button @click="nbSecondes += 10">
        Ajouter 10 secondes dans le timeout
      </button>
    </main>
  </div>
</template>
